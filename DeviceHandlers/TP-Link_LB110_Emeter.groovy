...
Supported models and functions:  This device supports the TP-Link LB110 with Emeter Functions.
...
metadata {
	definition (
		name: "TP-Link LB110 Emeter", namespace: "djg", author: "Dave Gutheinz") {
		capability "switch"
...
def energyMeterResponse(response) {
	if (response.headers["cmd-response"] == "TcpTimeout") {
		...
		log.info "$device.name $device.label: Udated CurrentPower to $powerConsumption"
		getUseToday()
	}
}
